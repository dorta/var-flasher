...
RUN apt-get update && apt-get install -y --no-install-recommends \
    ...
    util-linux udev sudo dbus-x11 libcanberra-gtk3-module fonts-noto-cjk \
...
CMD ["sh", "-c", "exec dbus-run-session -- node ./node_modules/electron/cli.js . --no-sandbox --disable-gpu --disable-gpu-compositing --in-process-gpu --ozone-platform=x11"]
